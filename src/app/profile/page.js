  ...
  return (
    <>
      ...
      <main className="max-w-3xl mx-auto px-4 py-10">
        ...
        {success && (
          <div className="mb-6 px-4 py-3 rounded-xl bg-green-50 border border-green-200 text-sm text-green-600">{success}</div>
        )}
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-xs text-gray-400 mb-1">用户名</p>
                {editing ? (
                  <input
                    type="text"
                    value={form.username}
                    onChange={(e) => setForm({ ...form, username: e.target.value })}
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none transition text-sm"
                  />
                ) : (
              ...
              <div className="flex-1">
                ...
                {editing ? (
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none transition text-sm resize-y"
                  />
                ) : (
                  <p className="text-gray-700 text-sm">{user.bio || '暂无简介'}</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-8">
          ...
          <div className="flex gap-2 mb-6">
            <input
              ...
              className="flex-1 px-3 py-2.5 rounded-xl border border-gray-200 focus:border-primary-400 focus:ring-2 focus:ring-primary-100 outline-none transition text-sm"
            />
            <button
              onClick={createApiKey}
              disabled={creatingKey || !newKeyName.trim()}
              className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-primary-600 text-white rounded-xl text-sm font-medium hover:bg-primary-700 transition disabled:opacity-50 shrink-0"
            >
          ...
          {copiedKey && (
            <div className="mb-4 px-4 py-3 rounded-xl bg-yellow-50 border border-yellow-200 text-sm text-yellow-800">
              ...
              <code className="block mt-1 text-xs bg-yellow-100 p-2 rounded break-all">{copiedKey}</code>
            </div>
          )}

          {apiKeys.length === 0 ? (
            <p className="text-sm text-gray-400 text-center py-6">暂无 API 密钥</p>
          ) : (
            <div className="space-y-2">
              {apiKeys.map((k) => (
                <div key={k.id} className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-xl">
                  <div>
        ...
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 text-red-500 rounded-xl border border-red-200 hover:bg-red-50 transition text-sm font-medium"
        >
